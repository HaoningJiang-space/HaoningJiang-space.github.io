---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

Education
======
* Ph.D. in Electronic and Computer Engineering, Hong Kong University of Science and Technology (HKUST), Fall 2026 (incoming)
* B.S. in Electrical and Electronic Engineering, Southern University of Science and Technology (SUSTech), 2022-2026 (expected)
  * GPA: 3.72/4.0 (90.14/100)
  * Core Courses: Methods in Mathematical Physics, Microwave Engineering, Wireless Communications, Optoelectronics Intellisense, Communication Principles, Solid-State Electronics, Data Communications and Networking

Work Experience
======
* **AI Algorithm Intern**, Ascend LLM Team, Hunan Kunpeng Ecological Innovation Research Center (HUAWEI)
  * Jan. 2025 - Mar. 2025, Changsha, Hunan, China
  * Built text-to-image and image generation pipelines using Diffusers framework with Stable Diffusion
  * Hosted technical workshops on reinforcement learning and post-training techniques (GRPO algorithm)
  * Implemented tiled matrix computation and online softmax, reducing GPU memory usage by 45%

Research Experience
======
* **Analog Circuit Design Automation**
  * Design-Intent-Driven Interactive Analog Layout Migration Framework, Peking University (Advisor: Prof. Yibo Lin)
  * Reinforcement Learning in Analog Circuit Sizing, SUSTech (Advisor: Prof. Junmin Jiang)
  * Achieved 5.2×-45.6× faster convergence using novel GRPO framework

* **Federated Learning and Medical AI**
  * Privacy-Preserving Federated Learning, The University of Hong Kong (Advisor: Prof. Liangqiong Qu)
  * Multimodal LLM for Brain Tumor Diagnosis, The University of Hong Kong (Advisor: Prof. Liangqiong Qu)

* **Computer Vision and Edge Computing**
  * AI-Powered SPICE Modeling Chart Analyzer, Tsinghua University (Advisor: Prof. Jian Song)
  * Real-Time Table Tennis Ball Tracking System, SUSTech (Advisor: Prof. Xihan Chen)

Skills
======
* **EDA Tools**: Cadence Virtuoso, Mentor Calibre, GDSTK
* **Deep Learning**: PyTorch, TensorFlow, Diffusers
* **Algorithms**: Stable Diffusion, GRPO, YOLO, Cascade R-CNN, Mask2Former, MAE, CLIP, SAM
* **Research**: Analog IC Design Automation, Reinforcement Learning, Federated Learning, Computer Vision

Publications
======
  <ul>{% for post in site.publications reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>

Awards & Fundings
======
* ASC25 World Supercomputing Competition: International Second Prize (May 2025)
* APAC HPC-AI Competition: Excellence Award (Top 8, Nov. 2024)
* ASC24 World Supercomputing Competition: International Second Prize (Apr. 2024)
* Computer System Design Competition: First Prize in South China Region
* Guangdong Climbing Project 2025: Principal Investigator, 20,000 RMB grant
* HKSTP2025: 100,000 RMB grant

Patents
======
* MARL Optimization Method for Analog Circuits and Related Devices (Invention)

Service and Leadership
======
* Reviewer and program committee member of AAAI2026, CVPR2026
